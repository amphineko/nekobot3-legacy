...
var Main = require('./../Bootloader2');
var querystring = require('querystring');
var Url = require('url');

var cookies, captcha_server;

// - Auth/Login.login(): QQ登錄入口函數, 僅在成功時調用Callback, 返回認證信息和Cookies
function login(config, callback) {
	log.info('<Auth/Login> 開始進行賬戶認證');
	console.log('即將登入帳號: ' + config.account);
	
	/* 檢查是否需要驗證碼 */
	checkCaptcha(config.account, function (ret, code, bits) {
		log.info('<Auth/Login> 完成確驗證碼狀態');
		console.log('Return ' + ret + ', Code ' + code + ', Bits ' + bits);
		
				/* 開始認證密鑰 */
		var next = function (nextcode) {
			authPassword(config.account, encodePassword(config.password, nextcode, bits), nextcode, function (ret) {
				if (!ret[2].match(/^http/)) {
					log.error('<Auth/Login> 密鑰認證失敗');
					Main.exit(71, 'Password Authorize Failed: ' + ret[4] + ' (' + ret[5] + ')');
				}
				var _nextjump = ret[2];
				ret = null;
				
				log.info('<Auth/Login> 初期化會話Cookies');
				/* 轉向騰訊回傳的下一跳位址, 獲取需要的Cookies */
				nextjump(_nextjump, function () {
					login2(function (ret, clientid, ptwebqq) {
						if (ret.retcode === 0) {
							log.info('<Auth/Login> 登入成功');
							var res = ret.result;
							var authblock = {
							...
							ret = null;
							console.log(
								'- Session account: ' + config.account + "\r\n" +
								'- Session clientid: ' + clientid + "\r\n" +
								'- Session uin: ' + authblock.uin);
							callback(cookies, authblock);
						} else {
							log.error('<Auth/Login> 登入失敗, 步驟channel/login2');
						}
					});
				});
			});
		...
		if (ret) {
			/* 要求用戶進行驗證碼認證 */
			authCaptcha(config.account, config.captcha.host, config.captcha.port, function (ret, res) {
...
// - Auth/Login.checkCaptcha(): 檢查賬戶認證碼狀態
function checkCaptcha(account, callback) {
	...
	https.get(options, function (resp) {
		...
		log.error('<Auth/Login> 確認驗證碼狀態失敗: ' + error);
		Bootloader.exit(10201, '確認驗證碼狀態失敗');
	});
}


// - Auth/Login.authCaptcha(): 進行驗證碼校驗
function authCaptcha(account, srvhost, srvport, callback) {
	var url = "http://captcha.qq.com/getimage?aid=1003903&r=" + Math.random() + "&uin=" + account;
	var body = '';
...
// - Auth/Login.waitCaptchaAuth(): 打開頁面等待用戶
...
// - Auth/Login.encodePassword(): 按照騰訊規則對密鑰進行處理, 供authPassword()使用
...
// - Auth/Login.authPassword(): 與騰訊認證賬戶密鑰
...
// - Auth/Login.nextjump(): 進行下一跳獲取Cookies
